...
K_ERR kReadyCtxtSwtch(K_TCB *const tcbPtr)
{
	...
	if (kTCBQEnq(&readyQueue[tcbPtr->realPrio], tcbPtr) == K_SUCCESS)
	{
		tcbPtr->priority = tcbPtr->realPrio;
		tcbPtr->status = READY;
		...
		TASKENTRY const taskFuncPtr)
{
	...
	stackAddrPtr[stackSize - PC_OFFSET] = (UINT32) taskFuncPtr;
	...
	{
		stackAddrPtr[stackSize - j] = 0xBADC0FFE;
	}
...
static inline VOID kReadyRunningTask_(VOID)
{
	...
	{
		assert(
				(kTCBQEnq(&readyQueue[runPtr->realPrio], runPtr) == (K_SUCCESS)));
